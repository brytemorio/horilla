...
var steps = [
    {popover: {title: 'Dashboard', description: 'Ems dashboard section'}},
    {
        element: '#attendance-activity-container',
        popover: {title: 'Mark Attendance', description: 'Used to mark your attendance'}
    },
];
...
if ($('#mainNavProfile').length) {
    steps.push({
        ...
        popover: {title: 'Profile', description: 'Profile and change password options'}
    });
}
if ($('.oh-card-dashboard').length) {
    steps.push({
        element: '#tileContainer .oh-card-dashboard:nth-child(1)',
        popover: {title: 'Dashboard Tiles', description: 'Ems Dashboard Tiles'}
    });
}
setTimeout(() => {
    if ($('.oh-btn-group').length) {
        steps.push({
            element: '.oh-btn-group:nth-child(1)',
            popover: {title: 'Dashboard Actions', description: 'Approve or Reject Options'}
        });
    }
    if ($('#addAnnouncement').length) {
        steps.push({
            element: '#addAnnouncement',
            popover: {title: 'Add announcement', description: 'Create announcement from dashboard'}
        });
    }
    ...
    if ($('.oh-sidebar__company').length) {
        steps.push({
            element: '.oh-sidebar__company:nth-child(1)',
            popover: {title: 'Company', description: 'Your current company access'}
        });
    }
    if ($('[data-id="dashboardNav"]').length) {
        steps.push({
            element: '[data-id="dashboardNav"]',
            popover: {title: 'App', description: 'Ems Hr Apps. eg Dashboard'}
        });
    }
}, 1000);
driverObj = driver(
