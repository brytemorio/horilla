...
try:
    ...
    ems_mail_templates = EmsMailTemplate.objects.all()
...
try:
    ...
    for penalty in penalties:
        ...
        if not PenaltyAccounts.objects.filter(**filter_conditions).exists():
            ...
            if apps.is_installed("attendance"):
                ems.late_early_id = penalty.late_early_id
            if apps.is_installed("leave"):
                ems.leave_request_id = penalty.leave_request_id
                ems.leave_type_id = penalty.leave_type_id
                ems.minus_leaves = penalty.minus_leaves
                ems.deduct_from_carry_forward = penalty.deduct_from_carry_forward
            ems.save()
